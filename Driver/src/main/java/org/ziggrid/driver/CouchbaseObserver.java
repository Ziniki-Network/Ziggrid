...
public class CouchbaseObserver { /*implements ZiggridObserver, BucketDelayLine.Processor<ViewProcessor, Object> {
	...
	public CouchbaseObserver(String couchUrl, String bucket, Model model, MessageSource messageSource) {
	...
	public CouchbaseObserver(String couchUrl, String bucket, ProcessingMethods pm, Model model, Collection<MessageSource> sources, int gateLength, TreeMap<String, Integer> gating) {
		this.couchUrl = couchUrl;
		this.bucket = bucket;
		this.pm = pm;
		this.model = model;
	...
	// TODO: this constructor needs a lot of work
	public CouchbaseObserver(CouchStorageConfig storage) {
		this.pm = null;
		this.gateLength = 0;
	}
	...
	public void deliver(ListMap<String, Interest> removeDead, Interest i, String ltype, JSONObject obj) {
		try {
			logger.error("Need to refactor this to use paylaods");
//			i.lsnr.deliver(obj);
		} catch (Exception ex) {
	...
	public boolean watch(ObserverListener sender, int h, String table, Map<String, Object> options) {
				...
				if (od.name.equalsIgnoreCase(table)) {
					try {
						...
						if (i.haveFullKey) {
							...
							if (curr != null)
//								try {
									logger.error("Need to refactor this to use payloads");
//									sender.deliver(new JSONObject(curr));
//								} catch (JSONException ex) {
